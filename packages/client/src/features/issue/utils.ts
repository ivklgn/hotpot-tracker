...
export const TASK_ISSUES_ID = 'task-issues';

export const focusCommentWithActiveId = (wrapperId: string, issueId: string) => {
  const wrapperEl = document.querySelector(`#${wrapperId}`);

  if (!wrapperEl) return;

  const issueInput = wrapperEl.querySelector<HTMLInputElement>(`input#${issueId}`);

  if (!issueInput) return;

  issueInput.scrollIntoView({
    behavior: 'smooth',
    block: 'center',
    inline: 'center',
  });
